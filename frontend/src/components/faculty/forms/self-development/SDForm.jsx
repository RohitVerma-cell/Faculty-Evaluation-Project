...
// ── Reusable TableForm with PDFUpload support ──
      ...
      {entries.map((entry, i) => (
        <div key={entry.id} style={styles.card}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>{title} #{i + 1}</h3>
              ...
              <div key={col.key} style={col.full ? styles.gridFull : {}}>
                {col.type === 'upload' ? (
                  ...
                  <>
                    ...
                    <select style={styles.input} value={entry[col.key] || col.options[0]}
                  ...
                  <>
                    <label style={styles.label}>{col.label}</label>
                    <input type={col.type || 'text'} style={styles.input}
                      placeholder={col.placeholder || ''} value={entry[col.key] || ''}
                      onChange={(e) => update(entry.id, col.key, e.target.value)} />
                  </>
                )}
              </div>
  ...
  return (
    <TableForm
      ...
      columns={[
        ...
        { key: 'numberOfDays',        label: 'Number of Days (3/4/5)',                   placeholder: 'e.g. 5' },
        { key: 'dates',               label: 'Dates of FDP',                             placeholder: 'DD/MM/YYYY - DD/MM/YYYY' },
        { key: 'proof',               label: 'Proof (PDF)',                  type: 'upload' },
      ]}
    />
  );
}
  ...
  return (
    <TableForm
      ...
      columns={[
        ...
        { key: 'provider',       label: 'MOOC Provider',                    placeholder: 'e.g. Coursera, NPTEL' },
...
// ── SD.5 PhD (no proof needed per document) ──
export function SD5Form({ data, setData }) {
  const supervisorRows   = data.phdCandidates   || [];
  const registrationRows = data.phdRegistration || [];

  const addSup = () => setData({ ...data, phdCandidates:   [...supervisorRows,   EMPTY({ candidateName: '', isBFGI: 'No', university: '', enrollmentNo: '', status: 'Thesis Submitted' })] });
  const addReg = () => setData({ ...data, phdRegistration: [...registrationRows, EMPTY({ university: '', enrollmentNo: '', status: 'Synopsis Submitted', supervisorBFGI: 'No' })] });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        {supervisorRows.map((row, i) => (
          <div key={row.id} style={{ ...styles.card, marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <span style={{ fontSize: 13, fontWeight: 600 }}>Candidate #{i + 1}</span>
            ...
            <div style={styles.grid}>
              ...
              <div><label style={styles.label}>Is candidate BFGI Faculty?</label>
                <select style={styles.input} value={row.isBFGI || 'No'} onChange={(e) => updSup(row.id, 'isBFGI', e.target.value)}>
              ...
              <div><label style={styles.label}>Status</label>
                <select style={styles.input} value={row.status || 'Thesis Submitted'}
                  onChange={(e) => updSup(row.id, 'status', e.target.value)}>
                  <option>Synopsis Submitted</option><option>Thesis Submitted</option><option>Degree Awarded</option>
                </select></div>
            </div>
          </div>
        ))}
      ...
      <div>
        ...
        {registrationRows.map((row, i) => (
          <div key={row.id} style={{ ...styles.card, marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <span style={{ fontSize: 13, fontWeight: 600 }}>Registration #{i + 1}</span>
            ...
            <div style={styles.grid}>
              ...
              <div><label style={styles.label}>Status</label>
                <select style={styles.input} value={row.status || 'Synopsis Submitted'}
                  onChange={(e) => updReg(row.id, 'status', e.target.value)}>
                  <option>Synopsis Submitted</option><option>Thesis Submitted</option><option>Degree Awarded</option>
                </select></div>
              <div><label style={styles.label}>PhD Supervisor is from BFGI?</label>
                <select style={styles.input} value={row.supervisorBFGI || 'No'}
...
export function SD6Form({ entries, setEntries }) {
  return (
    <TableForm
      ...
      emptyTemplate={{ awardTitle: '', organizationName: '', scope: 'National', dateOfAward: '', proof: null }}
      columns={[
