...
import { calcTLTotal, calcResearchTotal, calcSDTotal } from '../../utils/marksCalculator';

const FACULTY_EMAIL = 'Rishamjot12@gamil.com';
...
export default function DataEntryPage({
  ...
  const getPayload = () => ({
    facultyName: FACULTY_NAME, facultyEmail: FACULTY_EMAIL, academicYear: ACADEMIC_YEAR,
  ...
  const saveToBackend = async () => {
    const res = await fetch(BASE_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(getPayload()) });
  ...
  const handleSubmit = async () => {
    ...
    try {
      await saveToBackend();
      const res = await fetch(`${BASE_URL}/${FACULTY_EMAIL}/submit`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ facultyName: FACULTY_NAME }) });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{
        ...
        padding: '16px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
              ...
              <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 500 }}>{itemLabel}</span>
            </div>
            <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: '#0f172a', letterSpacing: '-0.4px' }}>{itemLabel}</h1>
          </div>
        </div>

        {/* Right — Section Score + Buttons */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 20, fontWeight: 800, color: meta.color, lineHeight: 1.1 }}>
              {currScore.marks.toFixed(1)}
            </span>
            <span style={{ fontSize: 10, color: meta.color, opacity: 0.75, fontWeight: 500 }}>
              / {currScore.max} marks
            </span>
          </div>
          ...
          <button onClick={handleSaveDraft} disabled={loading}
            style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 18px', borderRadius: 9, border: '1.5px solid #e2e8f0', background: '#fff', fontSize: 13, fontWeight: 500, color: '#475569', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1 }}>
